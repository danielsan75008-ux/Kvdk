...
-- Cria UMA part de hitbox centralizada e soldada a uma target part
local function makeHitboxPart(char, target, size, alpha)
    ...
    p.Parent          = char
    -- Posiciona no centro da target APÓS ser parentado
    p.CFrame          = target.CFrame

    -- Weld com C0/C1 zerados = centralizado exatamente no centro da target
    -- Part0 = target (personagem), Part1 = hitbox
    -- C0 = offset no espaço de Part0 = zero (centro)
    -- C1 = offset no espaço de Part1 = zero (centro)
...
local function applyHitbox(player)
    ...
    local alpha        = State.HitboxAlpha or 0.5
    ...
    if selectedName == "Corpo Inteiro (Root)" then
        ...
        -- Detecta rig e usa a lista certa
        ...
        -- Fallback custom rig: pega todas as BaseParts não-acessório
        if #targets == 0 then
            for _, v in ipairs(char:GetDescendants()) do
                if v:IsA("BasePart") and v.Name ~= "HitboxExpand" then
                    table.insert(targets, v)
                end
            end
        end
    else
        -- Parte específica selecionada
    ...
    for _, target in ipairs(targets) do
        local p = makeHitboxPart(char, target, size, alpha)
