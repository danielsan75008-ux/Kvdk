...
local function findAnchorPart(char, selectedName)
    ...
    for _, fb in ipairs({"HumanoidRootPart","Torso","UpperTorso","LowerTorso","Head"}) do
...
-- Guarda tamanhos originais para restaurar
local originalSizes = {}   -- originalSizes[part] = Vector3
...
local function applyHitbox(player)
    ...
    local char = player.Character
    if not char then return end
    -- Espera o character carregar completamente
    ...
    for _, part in ipairs(targets) do
        pcall(function()
            -- Guarda tamanho original
            originalSizes[part] = part.Size
            -- Expande a part existente — assim raycasts do servidor batem nela
...
-- Conecta hitbox + ESP no CharacterAdded de um player
local function connectCharacterEvents(pl)
    -- Conecta para respawns futuros
    pl.CharacterAdded:Connect(function(char)
        -- Espera o character estar totalmente carregado
        char:WaitForChild("HumanoidRootPart", 10)
        local hum = char:WaitForChild("Humanoid", 10)
        if hum then hum:GetPropertyChangedSignal("Health"):Wait() end
        task.wait(0.2)

        -- Reconstrói ESP para esse player
        buildESP(pl)

        -- Aplica hitbox se ativo
        if State.HitboxEnabled then
            task.wait(0.3)
            applyHitbox(pl)
        end
        -- Aplica chams se ativo
...
-- Inicializa para players já no jogo
for _, pl in ipairs(Players:GetPlayers()) do
    if pl ~= LocalPlayer then
        buildESP(pl)
        connectCharacterEvents(pl)
        -- Se hitbox já está ativo e o player já tem char, aplica imediatamente
