...
-- ══════════════════════════════════════════════════════
--  HITBOX PROFISSIONAL
--  - Part invisível separada soldada ao anchor
--  - NÃO modifica o personagem original
--  - SelectionBox visual controlado pelo slider
--  - Transparência 0 = laranja visível, 1 = invisível
-- ══════════════════════════════════════════════════════
local hitboxData = {}   -- [player] = { parts={Part}, boxes={SelectionBox} }
...
-- Calcula a transparência correta da Part a partir do slider
-- alpha=0 → laranja visível (Transparency=0)
-- alpha=1 → totalmente invisível (Transparency=1)
local function alphaToTransparency(alpha)
...
local function removeHitbox(player)
    ...
    for _, sb in ipairs(d.boxes) do
        pcall(function() sb:Destroy() end)
    end
    hitboxData[player] = nil
end

-- Cria uma Part hitbox invisível soldada a um anchor
local function makeHitboxUnit(char, anchor, size, alpha)
    local col = Color3.fromRGB(255, 140, 0)  -- laranja

    -- Part real que o servidor detecta
    local p = Instance.new("Part")
    p.Name         = "_HB"
    p.Size         = Vector3.new(size, size, size)
    p.CFrame       = anchor.CFrame          -- começa no centro do anchor
    p.Anchored     = false
    p.CanCollide   = false
    p.Massless     = true
    p.Transparency = alphaToTransparency(alpha)
    p.Color        = col
    p.Material     = Enum.Material.SmoothPlastic
    p.CastShadow   = false
    p.Parent       = char                   -- filho do char → segue a física

    -- Weld: Part0=anchor, Part1=p
    -- C0 e C1 zero = p fica exatamente centrado no anchor
    ...
    -- SelectionBox que adorna a Part hitbox (não o personagem)
    local sb = Instance.new("SelectionBox")
    sb.Adornee             = p
    sb.Color3              = col
    sb.LineThickness       = 0.04
    sb.SurfaceColor3       = col
    sb.SurfaceTransparency = alphaToTransparency(alpha)
    sb.Parent              = workspace
...
local function applyHitbox(player)
    ...
    local alpha = State.HitboxAlpha or 1      -- padrão invisível
    local sel   = State.HitboxPart  or "Corpo Inteiro (Root)"

    -- Determina quais anchors usar
    local anchors = {}
    if sel == "Corpo Inteiro (Root)" then
        ...
        if #anchors == 0 then
            for _, v in ipairs(char:GetDescendants()) do
                if v:IsA("BasePart") then table.insert(anchors, v) end
            end
        end
    else
        local a = findAnchorPart(char, sel)
        if a then table.insert(anchors, a) end
    end
    ...
    for _, anchor in ipairs(anchors) do
        pcall(function()
            local p, sb = makeHitboxUnit(char, anchor, size, alpha)
            table.insert(d.parts, p)
            table.insert(d.boxes, sb)
        end)
    end
...
do
    ...
    TabCombat:Slider({
        Title = "Transparência",
        Desc  = "0 = laranja visível  |  1 = totalmente invisível",
        Step  = 0.05,
        Value = { Min = 0, Max = 1, Default = 1 },
        Callback = function(v)
            State.HitboxAlpha = v
            -- Atualiza em tempo real sem precisar reaplica
            for _, d in pairs(hitboxData) do
                for _, p in ipairs(d.parts) do
                    pcall(function() p.Transparency = v end)
                end
                for _, sb in ipairs(d.boxes) do
                    pcall(function() sb.SurfaceTransparency = v end)
                end
            end
        end,
    })
