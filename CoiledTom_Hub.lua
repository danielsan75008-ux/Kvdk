    ...
    for _, e in ipairs(entries) do
        pcall(function()
            if e.part and e.part.Parent then
                ...
                e.part.CanCollide     = e.origCollide
...
local function applyHitbox(player)
    ...
    for _, part in ipairs(targets) do
        pcall(function()
            ...
            -- Expande o tamanho (dano/raycast detecta)
            part.Size         = Vector3.new(size, size, size)
            part.Color        = col
            part.Transparency = alpha

            -- CRÍTICO: desliga colisão física para NÃO travar o player
            -- e NÃO colidir com sua própria hitbox
            part.CanCollide   = false
            part.Massless     = true

            -- Collision group como camada extra de segurança
            pcall(function()
...
-- Loop que mantém as parts expandidas SEMPRE centradas no inimigo
-- Isso garante que a hitbox funciona de dentro e de fora
local hitboxSyncConn = nil
local function startHitboxSync()
    if hitboxSyncConn then return end
    hitboxSyncConn = RunService.Heartbeat:Connect(function()
        if not State.HitboxEnabled then return end
        for pl, entries in pairs(hitboxData) do
            ...
            for _, e in ipairs(entries) do
                pcall(function()
                    if e.part and e.part.Parent then
                        -- Garante que a part continua centrada na posição original
                        -- Isso faz funcionar mesmo quando você está dentro da hitbox
                        e.part.CFrame = e.part.CFrame  -- força atualização da física
                        e.part.CanCollide   = false
                        e.part.Massless     = true
                        e.part.Size         = Vector3.new(
                            State.HitboxSize,
                            State.HitboxSize,
                            State.HitboxSize
                        )
...
local function stopHitboxSync()
    if hitboxSyncConn then hitboxSyncConn:Disconnect(); hitboxSyncConn = nil end
end
