...
-- ══════════════════════════════════════════════════════
--  HITBOX — usa SelectionBox (cor sólida, instantâneo)
--  Expande o tamanho real das parts via Size
-- ══════════════════════════════════════════════════════
local hitboxData    = {}   -- [player] = { parts={}, origSizes={}, boxes={} }
...
local function removeHitbox(player)
    ...
    for i, part in ipairs(d.parts) do
        pcall(function()
            if d.origSizes[i] then
                part.Size = d.origSizes[i]
            end
        end)
    end
...
local function applyHitbox(player)
    ...
    -- Aguarda HumanoidRootPart sem yield longo
    local hrp = char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end

    local size = State.HitboxSize or 5
    local sel  = State.HitboxPart or "Corpo Inteiro (Root)"
    local col  = Color3.fromRGB(255, 140, 0)  -- laranja fixo

    -- Monta lista de parts alvo
    local targets = {}
    if sel == "Corpo Inteiro (Root)" then
        ...
        local anchor = findAnchorPart(char, sel)
        if anchor then table.insert(targets, anchor) end
    end

    if #targets == 0 then return end

    local d = { parts={}, origSizes={}, boxes={} }

    for _, part in ipairs(targets) do
        pcall(function()
            -- 1) Guarda tamanho original e expande (dano real)
            table.insert(d.parts, part)
            table.insert(d.origSizes, part.Size)
            part.Size = Vector3.new(size, size, size)

            -- 2) SelectionBox visual (cor laranja, transparência)
            local sb = Instance.new("SelectionBox")
            sb.Adornee             = part
            sb.Color3              = col
            sb.LineThickness       = 0.03
            sb.SurfaceColor3       = col
            sb.SurfaceTransparency = 1 - (State.HitboxAlpha or 0.5)
            sb.Parent              = workspace
...
do
    ...
    TabCombat:Slider({
        Title = "Transparência",
        Desc  = "0 = opaco  |  1 = invisível",
        Step  = 0.05,
        Value = { Min = 0, Max = 1, Default = 0.5 },
        Callback = function(v)
            State.HitboxAlpha = v
            -- Atualiza SelectionBoxes em tempo real
            for _, d in pairs(hitboxData) do
                for _, sb in ipairs(d.boxes) do
                    pcall(function()
                        sb.SurfaceTransparency = 1 - v
                    end)
                end
            end
        end,
